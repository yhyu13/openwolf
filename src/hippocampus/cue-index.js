 ...
 */
export function loadIndex(hippocampusPath) {
    try {
 ...
 */
export function saveIndex(hippocampusPath, index) {
 ...
 */
export function addEventToIndex(index, event) {
    // Location index
    for (const file of event.context.files_involved) {
        if (!index.location_index[file]) {
            index.location_index[file] = [];
...
export function removeEventFromIndex(index, eventId) {
    ...
    for (const path of Object.keys(index.location_index)) {
        index.location_index[path] = index.location_index[path].filter((id) => id !== eventId);
    ...
    for (const tag of Object.keys(index.tag_index)) {
        index.tag_index[tag] = index.tag_index[tag].filter((id) => id !== eventId);
    ...
    for (const path of Object.keys(index.trauma_index.by_path)) {
        index.trauma_index.by_path[path] = index.trauma_index.by_path[path].filter((id) => id !== eventId);
 ...
 */
export function indexNeedsRebuild(index, eventCount) {
    if (index === null)
        return true;
